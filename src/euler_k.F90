...
!
 ...
 real(mykind) :: fk1,fk2,fk3,fk4,fk5
 real(mykind) :: ev1,ev2,ev3,ev4,ev5
!
!real(mykind),dimension(6,1-ng:nx+ng) :: u,v
!real(mykind),dimension(6,ng,1-ng:nx+ng) :: uv
 real(mykind) :: uuk,uukp,uukp2
 real(mykind) :: vvk,vvkp,vvkp2
 real(mykind) :: wwk,wwkp,wwkp2
 real(mykind) :: ppk,ppkp,ppkp2
 real(mykind) :: entk,entkp,entkp2
 real(mykind) :: uv_1_1_k,uv_1_2_k
 real(mykind) :: uv_2_1_k,uv_2_2_k
 real(mykind) :: uv_3_1_k,uv_3_2_k
 real(mykind) :: uv_4_1_k,uv_4_2_k
 real(mykind) :: uv_5_1_k,uv_5_2_k
 real(mykind) :: uv_6_1_k,uv_6_2_k
 real(mykind) :: uv_1_1_km,uv_1_2_km
 real(mykind) :: uv_2_1_km,uv_2_2_km
 real(mykind) :: uv_3_1_km,uv_3_2_km
 real(mykind) :: uv_4_1_km,uv_4_2_km
 real(mykind) :: uv_5_1_km,uv_5_2_km
 real(mykind) :: uv_6_1_km,uv_6_2_km
...
!real(mykind),dimension(6) :: ft,uvs
 ...
 integer :: endi,endj,endk,kstart
!
 integer :: i,j,k,l,m,n
 integer :: km,kp,ll,lmax,mm,ng2
...
!  l=1
   k = -1
   rhom     = w_gpu(1,i,j,k)  +w_gpu(1,i,j,k+1)
   uuk      = w_gpu(2,i,j,k)  /w_gpu(1,i,j,k)
   uukp     = w_gpu(2,i,j,k+1)/w_gpu(1,i,j,k+1)
   vvk      = w_gpu(3,i,j,k)  /w_gpu(1,i,j,k)
   vvkp     = w_gpu(3,i,j,k+1)/w_gpu(1,i,j,k+1)
   wwk      = w_gpu(4,i,j,k)  /w_gpu(1,i,j,k)
   wwkp     = w_gpu(4,i,j,k+1)/w_gpu(1,i,j,k+1)
   ppk      = w_gpu(1,i,j,k)  *temperature_gpu(i,j,k)
   ppkp     = w_gpu(1,i,j,k+1)*temperature_gpu(i,j,k+1)
   entk     = (w_gpu(5,i,j,k)+ppk)/w_gpu(1,i,j,k)
   entkp    = (w_gpu(5,i,j,k+1)+ppkp)/w_gpu(1,i,j,k+1)
   uv_1_1_k = (wwk+wwkp)*(             2._mykind)*rhom
   uv_2_1_k = (wwk+wwkp)*(uuk+uukp)*rhom
   uv_3_1_k = (wwk+wwkp)*(vvk+vvkp)*rhom
   uv_4_1_k = (wwk+wwkp)*(wwk+wwkp)*rhom
   uv_5_1_k = (wwk+wwkp)*(entk+entkp)*rhom
   uv_6_1_k = (   +2._mykind  )*(ppk+ppkp)
!  l=2
   rhom     = w_gpu(1,i,j,k)+w_gpu(1,i,j,k+2)
   uukp2    = w_gpu(2,i,j,k+2)/w_gpu(1,i,j,k+2)
   vvkp2    = w_gpu(3,i,j,k+2)/w_gpu(1,i,j,k+2)
   wwkp2    = w_gpu(4,i,j,k+2)/w_gpu(1,i,j,k+2)
   ppkp2    = w_gpu(1,i,j,k+2)*temperature_gpu(i,j,k+2)
   entkp2   = (w_gpu(5,i,j,k+2)+ppkp2)/w_gpu(1,i,j,k+2)
   uv_1_2_k = (wwk+wwkp2)*(             2._mykind)*rhom
   uv_2_2_k = (wwk+wwkp2)*(uuk+uukp2)*rhom
   uv_3_2_k = (wwk+wwkp2)*(vvk+vvkp2)*rhom
   uv_4_2_k = (wwk+wwkp2)*(wwk+wwkp2)*rhom
   uv_5_2_k = (wwk+wwkp2)*(entk+entkp2)*rhom
   uv_6_2_k = (       2._mykind)*(ppk+ppkp2)
!
...
!    Right state (node i+1)
     rip       =  1._mykind/w_gpu(1,i,j,kp)
     up        =  w_gpu(2,i,j,kp) * rip
     vp        =  w_gpu(3,i,j,kp) * rip
     wp        =  w_gpu(4,i,j,kp) * rip
     qqp       =  0.5_mykind * (up*up  +vp*vp +wp*wp)
!    ppp       =  gm1 * (w_gpu(5,i,jp,k) - w_gpu(1,i,jp,k) * qqp)
!    hp        =  (w_gpu(5,i,jp,k)  + ppp) * rip
     hp        =  gamma*w_gpu(5,i,j,kp)*rip-gm1*qqp
!    average state
     r         =  w_gpu(1,i,j,kp)*ri
     r         =  sqrt(r)
...
!
     b2     =   gm1/cc
     b1     =   b2 * qq
     el_1_1 =   0.5_mykind * (b1      + ww* ci)
     el_2_1 =  -0.5_mykind * (b2 * uu         )
     el_3_1 =  -0.5_mykind * (b2 * vv         )
     el_4_1 =  -0.5_mykind * (b2 * ww +     ci)
     ...
     el_4_2 =   b2 * ww
     el_5_2 =  -b2
     el_1_3 =   0.5_mykind * (b1     - ww * ci)
     el_2_3 =  -0.5_mykind * (b2 * uu         )
     el_3_3 =  -0.5_mykind * (b2 * vv         )
     el_4_3 =  -0.5_mykind * (b2 * ww -     ci)
...
!
     ...
     pp   = rho*temperature_gpu(i,j,ll)
     fk1  =       w_gpu(4,i,j,ll)
     fk2  = ww *  w_gpu(2,i,j,ll)
     fk3  = ww *  w_gpu(3,i,j,ll)
     fk4  = ww *  w_gpu(4,i,j,ll)  + pp
     fk5  = ww * (w_gpu(5,i,j,ll)  + pp)
!
     wc = 0._mykind
     gc = 0._mykind
     wc = wc + el_1_1 * w_gpu(1,i,j,ll)
     gc = gc + el_1_1 * fk1
     wc = wc + el_2_1 * w_gpu(2,i,j,ll)
     gc = gc + el_2_1 * fk2
     wc = wc + el_3_1 * w_gpu(3,i,j,ll)
     gc = gc + el_3_1 * fk3
     wc = wc + el_4_1 * w_gpu(4,i,j,ll)
     gc = gc + el_4_1 * fk4
     wc = wc + el_5_1 * w_gpu(5,i,j,ll)
     gc = gc + el_5_1 * fk5
     ...
     wc = 0._mykind
     gc = 0._mykind
     wc = wc + el_1_2 * w_gpu(1,i,j,ll)
     gc = gc + el_1_2 * fk1
     wc = wc + el_2_2 * w_gpu(2,i,j,ll)
     gc = gc + el_2_2 * fk2
     wc = wc + el_3_2 * w_gpu(3,i,j,ll)
     gc = gc + el_3_2 * fk3
     wc = wc + el_4_2 * w_gpu(4,i,j,ll)
     gc = gc + el_4_2 * fk4
     wc = wc + el_5_2 * w_gpu(5,i,j,ll)
     gc = gc + el_5_2 * fk5
     ...
     wc = 0._mykind
     gc = 0._mykind
     wc = wc + el_1_3 * w_gpu(1,i,j,ll)
     gc = gc + el_1_3 * fk1
     wc = wc + el_2_3 * w_gpu(2,i,j,ll)
     gc = gc + el_2_3 * fk2
     wc = wc + el_3_3 * w_gpu(3,i,j,ll)
     gc = gc + el_3_3 * fk3
     wc = wc + el_4_3 * w_gpu(4,i,j,ll)
     gc = gc + el_4_3 * fk4
     wc = wc + el_5_3 * w_gpu(5,i,j,ll)
     gc = gc + el_5_3 * fk5
     ...
     wc = 0._mykind
     gc = 0._mykind
     wc = wc + el_1_4 * w_gpu(1,i,j,ll)
     gc = gc + el_1_4 * fk1
     wc = wc + el_2_4 * w_gpu(2,i,j,ll)
     gc = gc + el_2_4 * fk2
     wc = wc + el_3_4 * w_gpu(3,i,j,ll)
     gc = gc + el_3_4 * fk3
     wc = wc + el_4_4 * w_gpu(4,i,j,ll)
     gc = gc + el_4_4 * fk4
     wc = wc + el_5_4 * w_gpu(5,i,j,ll)
     gc = gc + el_5_4 * fk5
     ...
     wc = 0._mykind
     gc = 0._mykind
     wc = wc + el_1_5 * w_gpu(1,i,j,ll)
     gc = gc + el_1_5 * fk1
     wc = wc + el_2_5 * w_gpu(2,i,j,ll)
     gc = gc + el_2_5 * fk2
     wc = wc + el_3_5 * w_gpu(3,i,j,ll)
     gc = gc + el_3_5 * fk3
     wc = wc + el_4_5 * w_gpu(4,i,j,ll)
     gc = gc + el_4_5 * fk4
     wc = wc + el_5_5 * w_gpu(5,i,j,ll)
     gc = gc + el_5_5 * fk5
...
!
     ...
     omm1 = alfm1/summ
!
     vminus1 = omp0 *(-vp_weno1_1+3*vp_weno1_2) + omp1 *( vp_weno1_2+ vp_weno1_3)
     vplus1  = omm0 *(-vm_weno1_4+3*vm_weno1_3) + omm1 *( vm_weno1_2+ vm_weno1_3)
! m=2
...
!
     ...
     omm1 = alfm1/summ
!
     vminus2 = omp0 *(-vp_weno2_1+3*vp_weno2_2) + omp1 *( vp_weno2_2+ vp_weno2_3)
     vplus2  = omm0 *(-vm_weno2_4+3*vm_weno2_3) + omm1 *( vm_weno2_2+ vm_weno2_3)
! m=3
...
!
     ...
     omm1 = alfm1/summ
!
     vminus3 = omp0 *(-vp_weno3_1+3*vp_weno3_2) + omp1 *( vp_weno3_2+ vp_weno3_3)
     vplus3  = omm0 *(-vm_weno3_4+3*vm_weno3_3) + omm1 *( vm_weno3_2+ vm_weno3_3)
! m=4
...
!
     ...
     omm1 = alfm1/summ
!
     vminus4 = omp0 *(-vp_weno4_1+3*vp_weno4_2) + omp1 *( vp_weno4_2+ vp_weno4_3)
     vplus4  = omm0 *(-vm_weno4_4+3*vm_weno4_3) + omm1 *( vm_weno4_2+ vm_weno4_3)
! m=5
...
!
     ...
     omm1 = alfm1/summ
!
     vminus5 = omp0 *(-vp_weno5_1+3*vp_weno5_2) + omp1 *( vp_weno5_2+ vp_weno5_3)
     vplus5  = omm0 *(-vm_weno5_4+3*vm_weno5_3) + omm1 *( vm_weno5_2+ vm_weno5_3)
!
...
!  Update net flux 
   do k=kstart,endk ! loop on the inner nodes
    km = k-1
    do m=1,5
     df = (fhat_gpu(m,i,j,k)-fhat_gpu(m,i,j,km))*dzitdz_gpu(k)
